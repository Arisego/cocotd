...
public:
	...
	CCTouch* m_touch;
private:
	int b_battle;			//battlemap用state标记 || 1 - 等待弹出菜单 2 - 菜单弹出，拒绝事件 3 - 等待地图对象选择并返回给draw_func(); || 4 || 5 || 6 技能接续 || 7 技能接续选择中 || 8 AI决策中
public:
	...
	int max_x,max_y;			// <地图边界 || 格坐标形式
	set<pair<int,int>> ts_last;

	//vector<int> m_viEnenmis;
	//map<int,int> m_miEnemis;									// <[Change] 存在意义不明 | 阵营相关将会转入BattleFiled控制
...
private:	
	void dps_range(CCPoint a_cp , set<pair<int,int>> &a_dt, int a_max);					 // <生成图形：移动|Block 

	void dps_ring(CCPoint a_cp, set<pair<int,int>> &a_dt, int a_min, int a_max);		 // <生成图形：环形
	void dps_rect(CCPoint a_cp, set<pair<int,int>> &a_dt, int a_max);					 // <生成图形：圆形
...
public:
	...
	bool move_control(int ax, int ay);
	void clean_cs();			// clean states;
	void clean_allcs();			// clean all state;
	void a_star();				//A*
...
public:
	...
	void ChangeAllFace(CCObject* aeSrc = NULL, CCObject* aeTar = NULL);

	void PauseAllActs();		// <停止所有需要停止的子节点
	void ResumeAllActs();		// <恢复

	void PauseForVod();			// <播放声音的暂停|只停止脚本
	void ResuemForVod();		// <恢复

	void clear_mcaTarget();		// <清楚mcatarget列表中的mihit值，防止可能的bug

	virtual void thread_update_b2wrold();

	void PlaceEChesses(Script* scp);

	void clear_Arrange();			// <清除单次判定的缓存

	BSearch* m_bSearch;				// <技能缓存

	bool InMap(int ax, int ay);					// <判断一个点是否在地图中
	map<pair<int,int>, int> mBlocks;			// <地图Block详细登记
	set<pair<int,int>>		mBlockSet;			// <地图被Block的粗略集合

private:
	void CalBlockCS();			// <计算被屏蔽的输入到CS_DIS

public:
	void LockCamera(CCObject* e);			// <瞬间移动镜头
	virtual void f_setcamara(CCObject* e);
