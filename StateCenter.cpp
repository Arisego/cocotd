...
bool StateCenter::f_save_file(const char* psz){
	do 
	{
		...
		PackElement->SetAttribute("type", "0");		// type = 0
		...
		RootElement->LinkEndChild(PackElement);
		PackElement->SetAttribute("type",3); // type = 3
		...
		PackElement = new TiXmlElement("pack");
		RootElement->LinkEndChild(PackElement);
		PackElement->SetAttribute("type",4); // type = 4
		...
		if(tcd){
			CCDICT_FOREACH(tcd,pElement)
			{
...

				{
					...
					t_icad->mSkillList->saveXml(LockElement);
		...
		PackElement = new TiXmlElement("pack");
		RootElement->LinkEndChild(PackElement);
		PackElement->SetAttribute("type", "1"); // type = 1
		...
		BattleField::sharedBattleField()->unsetSaveState();
		if(BattleField::sharedBattleField()->mbNeedSave){
			//if(GameManager::sharedLogicCenter()->ml->bm){
			//	m_tpsz = ;
			//	m_tscr = ;
			//	//il	=	GameManager::sharedLogicCenter()->ml->bm->m_itemlist;
			//}

			PackElement = new TiXmlElement("pack");
			RootElement->LinkEndChild(PackElement);
			PackElement->SetAttribute("type", "5");
			PackElement->SetAttribute("map",GameManager::sharedLogicCenter()->ml->bm->map_path.c_str());
			//PackElement->SetAttribute("script",GameManager::sharedLogicCenter()->ml->bm->scr_path.c_str());
			...
			PackElement->SetAttribute("turn",GameManager::sharedLogicCenter()->ml->m_iTurn);

			CCDictionary* tcd = GameManager::sharedLogicCenter()->ml->tm->m_itemlist;
			CCDictElement* tcde = NULL;
			CCDICT_FOREACH(tcd, tcde){
				EChesses* tec = (EChesses*) tcde->getObject();

				PngElement = new TiXmlElement("png");			//IN;
				PackElement->LinkEndChild(PngElement);

				PngElement->SetAttribute("name",tec->name);
				PngElement->SetAttribute("x", tec->pos.x);
				PngElement->SetAttribute("y", tec->pos.y);
				CCLog(">[StateCenter] Save Battle State:%s", tec->name.c_str());

				//////////////////////////////////////////////////////////////////////////
				// <保存Chara

				int tig = tec->group_id ;
				if(tig == 0x01){
					PngElement->SetAttribute("id", tec->m_pChara->m_iCharaID);
				}else
				{
					Chara* t_icad = tec->m_pChara;

					TiXmlElement *LockElement0 = new TiXmlElement("png");
					PngElement->LinkEndChild(LockElement0);

					LockElement0->SetAttribute("cid",t_icad->m_iCharaID);

					{
						TiXmlElement *LockElement = new TiXmlElement("lock");
						LockElement0->LinkEndChild(LockElement);
						LockElement->SetAttribute("type",0);
						for(map<string,int>::iterator it = t_icad->m_iiAttrs.begin(); it != t_icad->m_iiAttrs.end(); ++it){
							TiXmlElement *LockElement1 = new TiXmlElement("lock");
							LockElement->LinkEndChild(LockElement1);
					...
					LockElement0->SetAttribute("chp",t_icad->getvalue("chp"));
					LockElement0->SetAttribute("cmp",t_icad->getvalue("cmp"));
					CCLOG(">Dic Each...");
				}
...
bool StateCenter::g_load_file(){
	...
	BattleField::sharedBattleField()->mbNeedSave = false;
	CCArray* scrps = g_sp->initcs;
	for(int i = 0; i<scrps->count(); ++i){
		Script* t = (Script*) scrps->objectAtIndex(i);
		switch(t->getint("type")){
		...
		case(5):
			{
				BattleField::sharedBattleField()->Load_Script(t);
				break;
			}
		default:
			{
