...
void TOChara::RefreshData( int Id )
{
	...
	if(g_chara->m_viSkills.size()>0)
	{
		...
		for(int i = 0;i<m_number;i++){
			...
			t_sk->zbtype		=	stoi(t_ssm.at("type"));
...
{
	CCLog(">[TOChara] EquipPop | Popup the Equip.Begin.Check:%d", m_iCurrentEuip);
...
void TOChara::SkillPopup()
{
	...
	t_csSql = CCString::createWithFormat("select * from skill_list where itemid IN (%s) and type = %d",t_sMask.c_str(),t_zbtype);
