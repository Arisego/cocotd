...
namespace SCMLHelper
{
	...
	void File::Init(TiXmlNode *node, CCSpriterX *animator)
	{
	...
	void Folder::Init(TiXmlNode *node, CCSpriterX *animator)
	{
		TiXmlElement *element = node->ToElement();
		if (element)
		{
	...
	void Animation::Update(float dt)
	{
		...
		if (mTimer >= mfEnd)
		{
			...
			////[0803]
			//[PLAYSTATEDEBUG]
			//>[CCSpriterX]CCLog(">[CCSpriterX] miPlayTimes:[%s||>%s],%d,[%0.2f, %0.2f]",animato->msCur.c_str(),animato->msLast.c_str(),animato->miPlayTimes, mfBegin, mfEnd);
		...
		if (keyframeNext)
		{
			...
			while (mTimer >= nextTime)
			{
				//>[CCSpriterX]CCLog(">[CCSpriterX] Looping-[%0.2f,%0.2f,%0.2f]", animato->msCur.c_str(), mTimer, currTime, nextTime);
				mCurrKeyframe = next;
			...
			//>[CCSpriterX]CCLog(">[CCSpriterX] %s-[%0.2f,%0.2f,%0.2f]", animato->msCur.c_str(), mTimer, currTime, nextTime);
	...
	void Entity::PlayTarget(const char *name, float afBegin /*= 0.0*/, float afEnd/* = -1*/)
	{
		...
		CCLog(">[spx] Find Anime: %d", mCurrAnimation);
		Animation *animation = mAnimations[mCurrAnimation];
...
{
	initWithSpriteFrameName("blank.png");
...
static const char* sDirect[] = {"left","right","up","down", "left"};
...
void CCSpriterX::setDirect(int ai)
{
	miDirect = ai;
}
...
void CCSpriterX::HideAllChildren()
{
	CCObject* tco;
	//setVisible(false);
	CCARRAY_FOREACH(this->m_pChildren, tco){
		((CCSprite*) tco)->setVisible(false);
	}
}
