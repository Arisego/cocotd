	...
	case 0:				//transform to fa
		{
			...
			EventCenter::sharedEventCenter()->mQueuedTrans.push(make_pair(fia, make_pair((p.x+dtx)/PTM_RATIO, (p.y+dty)/PTM_RATIO)));
			break;
		}
...
bool TileMap::init()
{
	...
	rw = r * ts.width;
	rh = r * ts.height;
	int mapWidth = m_tilemap->getMapSize().width * m_tilemap->getTileSize() * mfMapScale.width;
	int mapHeight = m_tilemap->getMapSize().height * m_tilemap->getTileSize() * mfMapScale.height;
...
CCPoint TileMap::m_getViewc(CCPoint c){	// <iso tile --> map base
	CCSize ts = m_tilemap->getTileSize() * mfMapScale;
...
void TileMap::f_setcamara(CCObject* e){
	if(!e) return;
	Entiles* te = (Entiles*) e;
	m_sNaCam = te->GetFullName();
	this->runAction(TFollow::create(te, CCRectMake(-dx, -dy, rw, rh)));
}
...
void *thread_logicupdate(void *arg)
{
	
	while (1) {
		pthread_testcancel();
		if(	!GameManager::sharedGameManager()->isActivate()) {		
			break;
		}
		TileMap* ttm = GameManager::sharedLogicCenter()->ml->tm;
		//CCLog("[TileMap]logic_update || check_before");
		if(ttm) {
			ttm->thread_update_b2wrold();
		}
		Sleep(10);
	}
	return NULL;
...
pthread_t TileMap::log_pid;

void TileMap::initLogic()
{
	CCLog(">[TileMap]initLogic()");
	pthread_create (&log_pid, NULL, &thread_logicupdate, NULL);
}


void TileMap::reLeaseLogic()
{
	int *tret;
	CCLog(">[TileMap]ReleaseLogic() || begin");
	//GameManager::sharedGameManager()->Disable();
	pthread_cancel(log_pid);
	pthread_join(log_pid,(void**) &tret);
	CCLog(">[TileMap]ReleaseLogic() || end");
}


void TileMap::DeActive()
{
	CCLog(">[TileMap]DeActive()");
	mbActiveate = false;
	cancontrol = false;
	reLeaseLogic();
}

void TileMap::EnActive()
{
	initLogic();
	mbActiveate = true;
}
...
void TileMap::thread_update_b2wrold()
{
	...
	do 
	{
		if(	EventCenter::sharedEventCenter()->mQueuedTrans.size() == 0) break;

		pair<b2Body*, pair<float, float>> ts = EventCenter::sharedEventCenter()->mQueuedTrans.front();

		ts.first->SetTransform(b2Vec2(ts.second.first, ts.second.second), 0);
		EventCenter::sharedEventCenter()->mQueuedTrans.pop();
	} while (true);


	//CCLog("Begin:%f",mf_b2delta);
	float dt = mf_b2delta;
	if(dt == 0) return;
	if(dt < (1/SYS_FRAME)) return;
	mf_b2delta = 0.0;

	//CCLog("[TileMap]logic_update || update_world");
...
void TileMap::update_b2world(float dt){
	...
	if(!mbWorldChanged) return;
	mbWorldChanged = false;
	//CCLog(">[TileMap] update_beworld() | begin");
	...
	// <Update Collicd
	//CCLog("[TileMap]logic_update || update_collide:%d",miMapType);
	if(colse->bc.size() > 0){
	...
	//	((EChessComp*) t_ec->getComponent("controller"))->CheckAlive();
...
{
	//CCLog(">[TileMap] update()");
	if(m_controller){

		if(mfVx>0) m_controller->direc = MS_RIGHT;
		else if(mfVx<0) m_controller->direc = MS_LEFT;
		else if(mfVy>0) m_controller->direc = MS_UP;
		else if(mfVy<0) m_controller->direc = MS_DOWN; //test only , modify it in envent ;
