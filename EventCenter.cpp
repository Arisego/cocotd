...
LgStat* EventCenter::m_lgstat = NULL;		// <防御UI

bool EventCenter::mbDefendLock = false;		// <是否按下了防御键
...
void EventCenter::handlerightm(WPARAM wParam, LPARAM lParam){
	CCLog(">[EventCenter] handle_rignt_mouse");
	if(m_bm_cake)
...
void EventCenter::unsetBmCake(Cake* s){
	if(s == m_bm_cake){
		m_bm_cake = NULL;
	}
}
...
{
	CCLog(">[EventCenter] setBmCake() | Set Cake %d",s);
	m_bm_cake = s;
}
...
void EventCenter::DefendOn()
{
	mbDefendLock = true;
	if(m_lgstat){
		m_lgstat->SetDefend(mbDefendLock);
	}
}

void EventCenter::DefendOff()
{
	mbDefendLock = false;
	if(m_lgstat){
		m_lgstat->SetDefend(mbDefendLock);
...
void EventCenter::SetDefndLck(LgStat* alg /*= NULL*/)
{
	m_lgstat = alg;
}

void EventCenter::ShiftPush()
{
	if(GameManager::sharedGameManager()->state == GameManager::SceneId::SCENE_PLAY){
		CCLog(">[EventCenter] ShiftPush() | GS exists.");
		if(GameManager::sharedLogicCenter()->m_StageState < 3 ) return;
		if(!GameManager::sharedLogicCenter()->ml) return;

		GameManager::sharedLogicCenter()->ml->FastShift();
	}
}
