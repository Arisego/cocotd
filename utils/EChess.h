...
public:
	...
	virtual void DecideDirect(CCPoint cur, b2Vec2 &bv);
...
public:
	EChesses* meRS;		//		<伤害来源登记 || 死亡判定后使用

	float mfBaseRate, mfAdditRate;
	void MoveChessSk(float ax, float ay);	// <技能用
	virtual void FixPosition();

private:
	virtual void FixTar();		// <内部修正用
	bool mbMoved;				// <是否发起了移动
