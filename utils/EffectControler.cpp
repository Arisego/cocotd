...
{
	CCLog(">[EffectController] md_use_skill() | Begin.");
	//////////////////////////////////////////////////////////////////////////
	vdata.clear();
	m_src = NULL;
	...
	mbSkill = m_src->mSkillList->getSkill(a_skill_id);

	sp->re_init();
	sp->parse_string(mbSkill->msSideEffect);
	...
	m_iTarget	=	 mbSkill->miTarget;	
	m_iState	=	 1;
	m_sEffect	=	 mbSkill->msDeffect;
	miView		=	 iv;
	...
	else CCLog(">[EffectController] md_use_skill() | Empty ‘deffect’ from db.");

	sp->re_init();

	BattleField::sharedBattleField()->miLastSkillID = a_skill_id;
	CCLog(">[EffectController] md_use_skill() | End.");
	iv->get_target();
...
bool EffectControler::DerLock(){
	...
	for(int i = 0; i<m_caLocks->count(); ++i){
		...
		case(2):		// < 2 | 是否持有指定的buffer值
			{
				ret = ret&(m_src->BufferState(t_scp->getint("bid")) < t_scp->getint("value"));
				break;
			}
		case(3):		// < 3 | Buffer的State不得小于
			{
				ret = ret&(m_src->BufferState(t_scp->getint("bid")) >= t_scp->getint("value"));
				break;
			}
		case(4):		// < 4 | Buffer的State不得大于
			{
				ret = ret&(m_src->BufferState(t_scp->getint("bid")) <= t_scp->getint("value"));
				break;
			}
...
void EffectControler::SetCLock(Chara* ac, int aid)
{
	if(!ac) return;
	m_src = ac;

	BSkill* tbs = m_src->mSkillList->getSkill(aid);
