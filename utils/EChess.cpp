...
void EChesses::load_chara_dbsp( Script* sp )
{
	...
	if(!m_pChara){
		...
		m_pChara->retain();
		CCLog(">[EChesses] load_chara_dbsp() | You Have No Chara [ERROR]");
	}

	m_pChara->m_iiAttrs.clear();
	m_pChara->m_miiEquips.clear();
	m_pChara->m_viSkills.clear();
...
EChesses::EChesses(){	
	...
	AIComponent* acp = new AIComponent();
	acp->autorelease();
	addComponent(acp);
...
{
	CCPoint cur = pos;
	CCLog(">[EChesses] ChageFace:%f,%f||%f,%f",cur.x,cur.y,ac.x,ac.y);
	if(ac.x < 0) return;
	if(ac.y < 0) return;
	...
	CCLog(">[EChesses] Direct Change To:%s",sDirect[direc]);

}

void EChesses::MoveChessSk(float ax, float ay)
{
	CCLog(">[EChesses] MoveChessSk [%f,%f] -> [%f,%f]", pos.x, pos.y, ax, ay);
	m_bMoving = true;
	if(BattleField::sharedBattleField()->HasChess(ax, ay)) return;	
	BattleField::sharedBattleField()->ChessMoved(this, pos, ccp(ax,ay));
	...
	int val = abs(ax - cur.x) + abs(ay - cur.y);
	mfAdditRate = val * 2;
	...
	CCLog(">[EChesses] MoveChessSk() | Over.");
}

void EChesses::FixPosition()
{
	CCPoint p = GameManager::sharedLogicCenter()->ml->bm->m_getViewc(pos);
	//m_body->SetTransform(b2Vec2((p.x+GameManager::sharedLogicCenter()->ml->bm->dtx)/PTM_RATIO,(p.y+GameManager::sharedLogicCenter()->ml->bm->dty)/PTM_RATIO),0);

	EventCenter::sharedEventCenter()->mQueuedTrans.push(make_pair(m_body, make_pair((p.x+GameManager::sharedLogicCenter()->ml->bm->dtx)/PTM_RATIO, (p.y+GameManager::sharedLogicCenter()->ml->bm->dty)/PTM_RATIO)));
}

void EChesses::FixTar()
{
	if(!mbMoved) return;
	CCLog(">[EChess] FixTar() | %f,%f", tar.x, tar.y);
	CCPoint p = GameManager::sharedLogicCenter()->ml->bm->m_getViewc(tar);
	//m_body->SetTransform(b2Vec2((p.x+GameManager::sharedLogicCenter()->ml->bm->dtx)/PTM_RATIO,(p.y+GameManager::sharedLogicCenter()->ml->bm->dty)/PTM_RATIO),0);
	EventCenter::sharedEventCenter()->mQueuedTrans.push(make_pair(m_body, make_pair((p.x+GameManager::sharedLogicCenter()->ml->bm->dtx)/PTM_RATIO, (p.y+GameManager::sharedLogicCenter()->ml->bm->dty)/PTM_RATIO)));
	mbMoved = false;
}
