...
#define NUM_EFFECT		32			// <用于播放effect的源的个数
#define BUFFER_SIZE     4096       // 
#define BUFFER_NUM		512			// <根据具体的update的频率来决定一个性能上最好的值，最好是1s一次！！
...
public:
	...
	void DownEffect(const char* pszFilePath);	/* <降低音调并消除记录，防止同时播放太多同音效引起的爆音 */
	...
	bool UpdateOgg(const char* name,ALuint uiSource){
		...
		{
			CCLog(">>>>> The only place possible break. <<<<<<");
